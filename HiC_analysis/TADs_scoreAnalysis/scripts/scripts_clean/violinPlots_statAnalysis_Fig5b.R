...
inFiles = list.files("./",pattern=".*data_new.tsv.*")
print(inFiles)
...
for(inFile in inFiles)
{
    ...
    levels = list("NOnubAll", "SUMOnubAll", "NOnubQ1", "SUMOnubQ1", "NOnubQ2", "SUMOnubQ2", "NOnubQ3", "SUMOnubQ3", "NOnubQ4", "SUMOnubQ4")
    #levels = unique(data$condition)
    print(levels)

    colors = rep(c(rgb( 115,   155,    107, 1.0, names = NULL, maxColorValue = 255), rgb( 253, 190, 113, 1.0, names = NULL, maxColorValue = 255)),as.integer(length(levels)/2))
    custom_pairs <- list()
    j = 1
