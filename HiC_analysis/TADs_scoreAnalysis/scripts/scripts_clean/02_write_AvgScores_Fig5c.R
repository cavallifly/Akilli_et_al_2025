...
outFile <- "avgScores_trans1Dinterval_allChrom_allStatePairs_data_new.tsv"
...
for (pair_name in names(all_inter_dfs))
{
  ...
  outFile <- paste0("avg_scores_", pair_name, "_inter_data_new.tsv")
  ...
  # Filter for Q2 (bottom quantile)
  interQ2 <- current_df[current_df$av_score.x >= qNOnub[[2]] & current_df$av_score.x <= qNOnub[[3]],]
  print(paste0("Head of Q2 for ", pair_name))
  print(head(interQ2))

  # Filter for Q3 (bottom quantile)
  interQ3 <- current_df[current_df$av_score.x >= qNOnub[[3]] & current_df$av_score.x <= qNOnub[[4]],]
  print(paste0("Head of Q3 for ", pair_name))
  print(head(interQ3))
  ...
  write.table(cbind("NOnubQ2", interQ2$av_score.x), file = outFile, row.names = F, sep = "\t", append = T, quote = F, col.names = F)
  write.table(cbind("SUMOnubQ2", interQ2$av_score.y), file = outFile, row.names = F, sep = "\t", append = T, quote = F, col.names = F)
  write.table(cbind("NOnubQ3", interQ3$av_score.x), file = outFile, row.names = F, sep = "\t", append = T, quote = F, col.names = F)
  write.table(cbind("SUMOnubQ3", interQ3$av_score.y), file = outFile, row.names = F, sep = "\t", append = T, quote = F, col.names = F)  
