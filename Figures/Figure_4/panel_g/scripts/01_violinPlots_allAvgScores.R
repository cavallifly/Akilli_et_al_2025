...
inFiles = list.files("./",pattern="1Ddistances_cisAndTrans1Dinterval_all_domains_vs_all_domains.tab")
print(inFiles)
...
for(inFile in inFiles)
{
    ...
    allData <- read.table(inFile, header=T)
    print(head(allData))

    dataNOnub   <- data.frame(paste0(allData$statePair,"_Control"),allData$av_score_NOnub)
    colnames(dataNOnub)   <- c("condition","values")
    dataSUMOnub <- data.frame(paste0(allData$statePair,"_SUMO"),allData$av_score_SUMOnub)
    colnames(dataSUMOnub) <- c("condition","values")
    print(head(dataNOnub))
    print(head(dataSUMOnub))    
    allData <- rbind(dataNOnub,dataSUMOnub)
    print(head(allData))    
    colnames(allData) <- c("condition","values")
    print(head(allData))

    #toMatch <- c("Active_Active","PcG_Active","Null_Het","PcG_Het","Null_Null","Het_Het","PcG_PcG")
    toMatch <- c("PcG_PcG", "Active_Active", "Het_Het", "Null_Null", "Active_PcG", "PcG_Active","Active_Het", "Het_Active","Het_Null", "Null_Het")
    allData <- allData[grep(paste(toMatch,collapse='|'),allData$condition),]

    allData$condition <- gsub("WD_NOnub","Control",allData$condition)
    allData$condition <- gsub("WD_SUMOnub","SUMO",allData$condition)
    allData$condition <- gsub("Het_Active","Active_Het",allData$condition)
    allData$condition <- gsub("Null_Het","Het_Null",allData$condition)        
    ...
    #levels = c(unique(data[grep("_10000-",data$condition),]$condition),unique(data[-grep("_10000-",data$condition),]$condition))
    #levels = unique(data$condition)
    levels = list("PcG_PcG_Control","PcG_PcG_SUMO", "Active_Active_Control","Active_Active_SUMO", "Het_Het_Control","Het_Het_SUMO","Null_Null_Control","Null_Null_SUMO","PcG_Active_Control","PcG_Active_SUMO","Active_Het_Control","Active_Het_SUMO","Het_Null_Control","Het_Null_SUMO")
    print(levels)
    #quit()
    #levels = c(unique(data[grep("_0-",data$condition),]$condition),unique(data[-grep("_0-",data$condition),]$condition))    
    ...
    summ <- data %>%
           group_by(condition) %>%
	   reframe(n = n(), condition=condition, score = min(data$values)-2)
    summ <- unique(summ)
    summ <- summ[grep("Control",summ$condition),]
    print(as.data.frame(summ), quote=F)   
    #quit()

    #dataPoint <- read.table("plotted_pair_Fig5_PcG63_PcG65.bed", header=T)   
    bxp <- ggviolin(data, x="condition", y="values", color="black", fill="condition", alpha=.50, scale = "width", trim = TRUE) + scale_fill_manual(values=colors) +
	   ...
	   geom_text(data=summ, aes(x=condition, y=score, label = paste0("n = ",n)), position = position_nudge(x = 0.5), size=4., color="black") +    
	   theme_classic() +
