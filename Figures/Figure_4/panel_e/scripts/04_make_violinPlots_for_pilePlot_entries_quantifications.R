...
if(comparison == "betweenConditions")
{
  inFiles = list.files("./",pattern=".*allCond.*data.tab.*")
  conditionsOrder <- c("Control","SUMORNAi")
}
if(comparison == "betweenReplicates")
{
  inFiles = list.files("./",pattern=".*allRepl.*data.tab.*")
  conditionsOrder <- c("Rep1","Rep2")
}
statesOrder    <- c("All","PcG","Active","Het","Null")
print(inFiles)
...
for(inFile in inFiles)
{
	...
	{
	    levelsA <- levels[ grep(state, levels) ]
	    levelsB <- levelsA[ grep(cond,levelsA) ]
	    print(levelsB[[1]])
	    
	    newLevels <- c(newLevels,levelsB[[1]])
	    i = i + 1
	}
    }
    levels <- newLevels
    ...
    summ <- data %>%
           group_by(condition) %>%
	   reframe(n = n(), condition=condition, score = min(data$values)*(1-0.05))
    summ <- unique(summ)
    summ <- summ[grep(conditionsOrder[[1]],summ$condition),]    
