...
#inFiles = list.files("./",pattern=".*data.tsv.*")
inFiles = list.files("./",pattern="TopDom_domains_hic_WD_.*Onub_merge_dm6_NA_window_5_at_5000bp_filtered_refined_at_1000bp_with_WD_NOnub_and_SUMOnub_merge_.*insulation_data.tsv")
print(inFiles)
#quit()
...
for(inFile in inFiles)
{
    ...
    #toMatch <- c("PcG_PcG", "Active_Active", "Het_Het", "Null_Null", "PcG_Active","Het_Active","Null_Het")
    ...
    summ <- data %>%
           group_by(condition) %>%
	   reframe(n = n(), condition=condition, score = min(data$values)*(1-sign(min(data$values))*0.05))
    summ <- unique(summ)
