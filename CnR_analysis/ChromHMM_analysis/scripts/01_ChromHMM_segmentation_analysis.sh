...
do
    for binning in 200 5000 ;
    do
	outDir=analysis_${condition}_${analysisTag}_at_${binning}bp
	if [[ -d ${outDir} ]];
	then
	...
	signalDir=signal_${condition}_at_${binning}bp    
	...
	then
	    ...
	    java -mx40G -jar /home/Programs/ChromHMM/ChromHMM.jar BinarizeBam -b ${binning} -t ${signalDir} -p 0.0001 ./scripts/chrom_sizes.txt ./${bamDir}/ cellmarkfiletable_${analysisTag}.tsv ${binarizedBams} 
	...
	# Best guess is 3: active (K27ac); Polycomb (K27me3 and Pc); Null (None)
	for nstates in 4 ;		       
	do
	...
	mv -v m*_at_${binning}bp* s*_at_${binning}bp* b*_at_${binning}bp* o*_at_${binning}bp* ${outDir}
