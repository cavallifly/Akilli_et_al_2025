...
replicates=$(ls -1 | grep chipseq | sed "s,_, ,g" | awk '{for(i=1;i<=5;i++){printf("%s_",$i)};printf("%s_","all");for(i=7;i<=(NF-1);i++){printf("%s_",$i)};printf("%s\n",$NF)}' | uniq | grep Rep | grep -v all)
if [[ ${replicate} == "" ]];
then
    echo "There are no replicates sequenced in more than one lane. You can proceed with the step 03."
    exit
else
    echo "I am going to merge lane for the following replicates:"
    echo ${replicates}
fi
...
do
    ...
    bamFiles=$(ls -1 ${PWD}/$assayName_$targetName_$cellName_$condition_$replicateName_*_$assembly_$tag/*bam)
