...
if [[ ! -e readCounts_bamfiles.npz ]];
then
    echo "4 - Compare and clusterize the samples from the .bam files using deeptools"
    conda run -n deeptools multiBamSummary bins --bamfiles ./*Rep*/*.bam --minMappingQuality 30 -out readCounts_bamfiles.npz --outRawCounts readCounts_bamfiles.tab -v
...
if [[ ! -e heatmap_bamfiles.pdf ]];
then
    conda run -n deeptools plotCorrelation --corData readCounts_bamfiles.npz --corMethod spearman --skipZeros --plotTitle "Spearman Correlation of RNAseq samples" --whatToPlot heatmap --colorMap bwr --plotNumbers --plotFile heatmap_bamfiles.pdf --outFileCorMatrix SpearmanCorr_readCounts_bamfiles.tsv
