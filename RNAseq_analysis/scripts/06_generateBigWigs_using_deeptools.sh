...
do
    ...
    #effectiveGenomeSize=$(faCount /zdata/data/DBs/*/UCSC/${assembly}/Sequence/WholeGenomeFasta/genome.fa | grep total | awk '{print $2}')
    ...
    if [[ -e ${outBw} ]];
    then
        echo ".bigWig already present in ${replicate}!"
	cd .. # Exit ${replicate}
        continue
    fi

    echo "3 - Create .bigWig for ${replicate} using DeeTools"
    /home/michael.szalay/anaconda3/envs/deeptools/bin/bamCoverage --outFileFormat bigwig --ignoreDuplicates --normalizeUsing RPKM --numberOfProcessors max/2 --effectiveGenomeSize ${effectiveGenomeSize} -v --bam ${inBam} --outFileName ${outBw} 
