...
replicates=$(ls -1 | grep rnaseq | grep _all_ | sed "s,_, ,g" | awk '{for(i=1;i<=4;i++){printf("%s_",$i)};printf("%s_","merge");for(i=6;i<=(NF-1);i++){printf("%s_",$i)};printf("%s\n",$NF)}' | uniq)
echo ${replicates}
...
do
    ...
    bamFiles=$(ls -1 ${PWD}/${assayName}_${targetName}_${cellName}_${condition}_*${laneName}_${assembly}_${tag}/*bam)
    echo $bamFiles | awk '{for(i=1;i<=NF;i++){printf("%s\n", $i)}}'
    
    if [[ -d ${replicate} ]];
    then
	echo "Replicates already merged in ${replicate}!"
	continue
    fi
